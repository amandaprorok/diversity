...
def Print(x, f, accepted):
  ...
  current_iteration += 1
  output = '\r%i) Cost: %.4f, Accepted %s' % (current_iteration, f, str(accepted))
  sys.stdout.write(output)
  sys.stdout.write(' ' * (utils.GetTerminalWidth() - len(output)))
  sys.stdout.flush()
...
def Optimize(Y_desired, A, X_init, Q, max_rate,
  ...
  # Bound by max rate.
  cost_fun = lambda x: Cost(x, Y_desired, A, X_init, Q, alpha=alpha, specified_time=specified_time, beta=beta, nu=nu, mode=mode, margin=margin)
  bound_fun = lambda *args, **kargs: BoundParameters(num_nonzero_elements * num_species, max_rate, minimize_convergence_time, *args, **kargs)
  ...
  if verify_gradients:
    for i in range(10):
      ...
      CheckGradient(lambda x: Cost(x, Y_desired, A, X_init, Q, alpha=alpha, specified_time=specified_time, beta=beta, nu=nu, mode=mode, margin=margin),
                    gradient_parameters)
